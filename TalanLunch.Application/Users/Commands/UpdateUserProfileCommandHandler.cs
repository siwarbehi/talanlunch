...
namespace TalanLunch.Application.Users.Handlers
{
    public class UpdateUserProfileHandler : IRequestHandler<UpdateUserProfileCommand, UpdateUserProfileCommandResult>
    {
        ...
        public UpdateUserProfileHandler(IUserRepository userRepository,IMapper mapper,IBlobStorageService blobStorageService)
        {
        ...
        /* private async Task<string> SaveProfileImageAsync(IFormFile profilePicture)
         {
             ...
             {
                 throw new ArgumentException("Aucune image valide fournie.");
             }

             return await _blobStorageService.UploadFileAsync(profilePicture);
         }*/
        private async Task<string> SaveProfileImageAsync(IFormFile profilePicture)
        {
            if (profilePicture == null || profilePicture.Length == 0)
            ...
            string containerName = "userimages"; 
            return await _blobStorageService.UploadFileAsync(profilePicture, containerName);
        }
